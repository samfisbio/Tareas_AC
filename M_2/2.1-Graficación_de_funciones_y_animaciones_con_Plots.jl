...
# ╔═╡ b2b6b68c-346a-4f56-94b8-5cf885cea206
...
H = 2 #Altura inicial
θ = π/4 #Ángulo de lanzamiento
v_0 = 20 #Velocidad de lanzamiento inicial
G = 9.81 #Aceleración de la gravedad
t=
	
v_0x = v_0 * cos(θ) #Velocidad inicial en x
v_0y = v_0 * sin(θ) #Velocidad inicial en y
	
#Cada eje
x = v_0x*t #en x
y = H + v_0y*t - (1/2)*G*t^2 #en y

 #Para el tiempo de vuelo
T=(v_0/G)*(sin(θ)+sqrt((sin(θ))^2 +2*((G*H)/(v_0^2)))) 
x_m=(v_0^2/G)*(sin(θ)+sqrt((sin(θ))^2 +2*((G*H)/(v_0^2))))*cos(θ) 
y_m=((v_0^2)*sin(θ)^2)/(2*G)+H 

# Función de coordenada y de tiro parabólico para la línea punteada
function j(t) 
        return (H + v_0y*(t/v_0x) - (1/2)*G*(t/v_0x)^2)
end

 # Función de coordenada y de tiro parabólico para la figura 
s=H
function z(t)
        return s =+ (H + v_0y*(t/v_0x) - (1/2)*G*(t/v_0x)^2)
end

#Animación

@gif for t in (0:0.025:T)
	plot(j,v_0x*(0:0.01:t), xaxis=[0,x_m], yaxis=[0,y_m], style = [:dot], color = "pink", lj = [2], legend = false, title=("tiro parabólico"), xlabel=L"x", ylabel=L"y", background="white")
	scatter!(z,v_0x*t,v_0x*t, xaxis=[0,x_m], yaxis=[0,y_m], color ="pink", ms = [7], legend = false)
    end
end
...
# ╔═╡ c05f6273-ac97-48cb-b6b6-9b7a76843bd8
# Tu código (comentado) va aquí :D
...
h = 2 #Altura inicial
k = π/4 #Ángulo de lanzamiento
v_0 = 20 #Velocidad de lanzamiento inicial
G = 9.81 #Aceleración de la gravedad
t= 

v_0x = v_0 * cos(k) #Velocidad inicial en x
v_0y = v_0 * sin(k) #Velocidad inicial en y

#coordenadas
x = v_x*t 
y = h + v_y*t - (1/2)*G*t^2 

T_v=(v/G)*(sin(k)+sqrt((sin(k))^2 +2*((G*h)/(v^2)))) 
x_M=(v^2/G)*(sin(k)+sqrt((sin(k))^2 +2*((G*h)/(v^2))))*cos(k) 
y_M=((v^2)*sin(k)^2)/(2*G)+h 
...
c=h
function w_1(t)
        return c = + (h + v_0y*(t/v_0x) - (1/2)*G*(t/v_0x)^2)
end

#Animación

@gif for t in (0:0.025:T_v)
	plot(s_1,v_x*(0:0.01:t), xaxis=[0,x_M], yaxis=[0,y_M], style = [:dot], color = "pink", ls = [2], legend = false, title=("tiro parabólico"), xlabel=L"x", ylabel=L"y", background=white")
	scatter!(w_1,v_x*t,v_x*t, xaxis=[0,x_M], yaxis=[0,y_M], color = "white", ms = [7], legend = false)
    end
end
end
...
@gif for n in (0:0.1:3)
	surface((0:0.1:2π),(0:0.1:2*π*n),f,yaxis=[0,y_max],zaxis=[-3,3],background="white", color=:acton)
    end
